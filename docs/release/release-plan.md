...
- The OPS-007 resource/log gate must also pass before completion: `/api/status.operations` must not contain critical disk, memory, CPU, SQLite WAL, telemetry-cleanup, runtime-log, or retryable SQLite-lock alerts.
...
- 2026-05-17 resource/log keepalive monitoring: OPS-007 adds SQLite retry tracking, telemetry retention cleanup, resource snapshots for disk/memory/CPU/SQLite/WAL/log sizes, and metrics-worker SQLite-lock retry evidence to the same operations surface.

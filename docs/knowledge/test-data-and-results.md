...
- API settings validation test verifies unsafe keys and invalid port values are rejected, while `collection.wait_cycles=0` is accepted and read back safely.
...
- 2026-05-08 full-market planning smoke: `odm-collect --max-underlyings 1000000 --max-batches 3 --option-batch-size 20 --wait-cycles 1` materialized 380 underlyings, 27,386 options, and 1,544 active batches; selected batches completed successfully while source quality gaps remained visible.

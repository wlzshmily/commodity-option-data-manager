...
- Realtime freshness is delivered by `odm-quote-stream`, which keeps Quote and Kline subscription objects alive, writes current Quote snapshots, reports runtime subscription-object progress, and surfaces official TQSDK connection notifications for reconnect observability. API startup always refreshes the active contract universe before realtime workers start, and long-running workers run periodic contract refresh/reconciliation so newly active symbols are added and inactive symbols are removed without full worker restart. Full-market catch-up is delivered by process-level shards.

...
- Realtime freshness is delivered by `odm-quote-stream`, which keeps Quote and Kline subscription objects alive, writes current Quote snapshots, reports runtime subscription-object progress, and surfaces official TQSDK connection notifications for reconnect observability. API startup bootstraps the active contract universe with one TQSDK discovery pass when a new database has no active options; full-market catch-up is delivered by process-level shards.
